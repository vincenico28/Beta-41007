...
function AppraisalDetailDialog({
  ...
  const handlePrint = () => {
    if (printableRef.current) {
      const empName = review.employees ? `${review.employees.first_name} ${review.employees.last_name}` : 'Employee'
      printHtmlElement(printableRef.current, {
        title: `Performance Appraisal - ${empName}`,
  ...
  return (
    <Dialog open onOpenChange={open => { if (!open) onClose() }}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto p-0 border-border">
        {/* Header Ribbon (On-screen only) */}
        <div className="bg-gradient-to-r from-slate-900 via-primary/95 to-slate-900 text-white p-6 rounded-t-lg no-print">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="secondary" onClick={handlePrint} className="gap-1.5 bg-white/20 text-white hover:bg-white/30 border-white/30">
                <Printer className="size-3.5" /> Print Appraisal
              </Button>
            </div>
          </div>
        </div>

        {/* Complete Printable Appraisal Sheet */}
        <div ref={printableRef} className="print-area p-6 space-y-6 text-sm bg-white text-black">
          {/* Printable Official Header */}
          <div className="text-center space-y-1 pb-4 border-b-2 border-black">
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-700">Official DOLE & CSC Compliant Performance Appraisal</h2>
            <h1 className="text-xl font-black uppercase tracking-wider">PRIORITY HANDLING LOGISTICS, INC.</h1>
            <p className="text-xs text-gray-600">Air, Sea & Ground Logistics Operations &bull; Metro Manila, Philippines</p>
            <p className="text-sm font-bold uppercase pt-1">EMPLOYEE PERFORMANCE APPRAISAL & EVALUATION REPORT</p>
          </div>

          {/* Metadata Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-300">
            <div>
              <span className="text-[11px] text-gray-500 font-semibold uppercase">Employee Name</span>
              <p className="font-bold text-gray-900 mt-0.5">
                {review.employees ? `${review.employees.first_name} ${review.employees.last_name}` : 'Unknown'}
              </p>
            </div>
            <div>
              <span className="text-[11px] text-gray-500 font-semibold uppercase">Department / Role</span>
              <p className="font-bold text-gray-900 mt-0.5">
                {review.employees?.departments?.name || 'Logistics'} &bull; {review.employees?.position || 'Staff'}
              </p>
            </div>
            <div>
              <span className="text-[11px] text-gray-500 font-semibold uppercase">Review Period</span>
              <p className="font-bold text-gray-900 mt-0.5">
            ...
            <div>
              <span className="text-[11px] text-gray-500 font-semibold uppercase">Evaluator</span>
              <p className="font-bold text-gray-900 mt-0.5">
                {review.reviewer ? `${review.reviewer.first_name} ${review.reviewer.last_name}` : 'HR Management / Supervisor'}
              </p>
            </div>
          </div>

          {/* Overall Rating & Performance Tier Box */}
          <div className="rounded-xl border border-gray-300 bg-gray-50 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="size-16 rounded-xl bg-slate-900 text-white flex flex-col items-center justify-center font-bold shadow-md shrink-0">
                <span className="text-2xl leading-none">{overall.toFixed(1)}</span>
                <span className="text-[10px] opacity-80 mt-0.5">/ 5.0</span>
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-base text-gray-900">{tier.label}</span>
                  <div className="flex gap-0.5 text-amber-500">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span key={i}>{i < Math.round(overall) ? '★' : '☆'}</span>
                    ))}
                  </div>
                </div>
                <p className="text-xs text-gray-600 max-w-md">{tier.description}</p>
              </div>
            </div>
            <div className="text-right sm:border-l sm:border-gray-300 sm:pl-4">
              <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Payroll Merit Incentive</span>
              <p className={`text-sm font-bold mt-0.5 ${tier.bonusEligible ? 'text-emerald-700' : 'text-gray-600'}`}>
                {tier.bonusEligible ? `Eligible (${tier.bonusRate})` : 'Standard Performance'}
              </p>
            </div>
          </div>

          {/* Competency Ratings Grid Table */}
          <div className="space-y-2">
            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-700">Competency Evaluation Matrix (Weight: 20% each)</h3>
            <table className="w-full text-left text-xs border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100 font-bold border-b border-gray-300">
                  <th className="border border-gray-300 p-2">Core Competency Area</th>
                  <th className="border border-gray-300 p-2 w-28 text-center">Score (1–5)</th>
                  <th className="border border-gray-300 p-2 w-32 text-center">Performance Level</th>
                </tr>
              </thead>
              <tbody>
                {competencies.map(c => {
                  const level = c.rating >= 4.5 ? 'Exemplary' : c.rating >= 3.5 ? 'Proficient' : c.rating >= 2.5 ? 'Developing' : 'Needs Action'
                  return (
                    <tr key={c.name} className="border-b border-gray-200">
                      <td className="border border-gray-300 p-2 font-medium text-gray-900">{c.name}</td>
                      <td className="border border-gray-300 p-2 text-center font-bold font-mono text-gray-900">{c.rating.toFixed(1)} / 5.0</td>
                      <td className="border border-gray-300 p-2 text-center font-semibold text-gray-700">{level}</td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-3.5 rounded-lg border border-emerald-300 bg-emerald-50/60 space-y-1">
              <span className="text-xs font-bold text-emerald-900">Key Strengths</span>
              <p className="text-xs text-gray-700 leading-relaxed">
                {review.strengths || 'Demonstrates strong consistency and reliable work ethics in regular assigned functions.'}
              </p>
            </div>
            <div className="p-3.5 rounded-lg border border-amber-300 bg-amber-50/60 space-y-1">
              <span className="text-xs font-bold text-amber-900">Areas for Growth</span>
              <p className="text-xs text-gray-700 leading-relaxed">
                {review.improvements || 'Focus on cross-functional logistics synchronization and proactive issue reporting.'}
              </p>
            </div>
            <div className="p-3.5 rounded-lg border border-blue-300 bg-blue-50/60 space-y-1">
              <span className="text-xs font-bold text-blue-900">Next Quarter Objectives</span>
              <p className="text-xs text-gray-700 leading-relaxed">
          ...
          <div className="border-t border-gray-300 pt-4 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wider text-gray-700 flex items-center gap-1.5">
                ...
                Employee Sign-off & Acknowledgement Status
              </h3>
              <span className="text-xs font-bold uppercase px-2 py-0.5 rounded border border-gray-300 bg-gray-100">
                {review.status === 'acknowledged' ? 'Acknowledged & Certified' : 'Pending Formal Sign-off'}
              </span>
            </div>

            {review.status === 'acknowledged' ? (
              <div className="p-3.5 rounded-lg bg-emerald-50/60 border border-emerald-300 space-y-1.5 text-xs">
                <div className="flex items-center justify-between font-semibold text-emerald-900">
                  <span>Digitally Signed & Certified by Employee</span>
                ...
                {review.employee_comments && (
                  <p className="text-gray-700 italic bg-white p-2.5 rounded border border-emerald-200">
                    "{review.employee_comments}"
                  </p>
                )}
              </div>
            ) : canAcknowledge ? (
              <div className="p-4 rounded-xl border border-primary/30 bg-primary/[0.03] space-y-3 no-print">
              ...
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-300 text-xs text-gray-600 italic flex items-center gap-2">
          ...
          {/* Official Sign-off block for paper printouts */}
          <div className="pt-8 grid grid-cols-3 gap-6 text-xs text-center border-t border-black">
            <div className="border-t border-black pt-2">
              <p className="font-bold">{review.employees ? `${review.employees.first_name} ${review.employees.last_name}` : 'Employee'}</p>
              <p className="text-[10px] text-gray-600">Employee Signature & Date</p>
            </div>
            <div className="border-t border-black pt-2">
              <p className="font-bold">{review.reviewer ? `${review.reviewer.first_name} ${review.reviewer.last_name}` : 'Supervisor / Evaluator'}</p>
              <p className="text-[10px] text-gray-600">Immediate Supervisor Signature</p>
            </div>
            <div className="border-t border-black pt-2">
              <p className="font-bold">Human Resources Director</p>
              <p className="text-[10px] text-gray-600">Approved & Enforced into 201 File</p>
            </div>
          </div>
        </div>

        <DialogFooter className="p-4 border-t border-border bg-muted/20 no-print">
          <Button variant="outline" onClick={onClose}>Close</Button>
          <Button onClick={handlePrint} className="gap-1.5">
            <Printer className="size-4" /> Print Appraisal
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
