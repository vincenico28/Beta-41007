...
function IDCardDialog({ emp, open, onOpenChange }: { emp: Employee | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const handlePrint = () => {
    if (printableRef.current) {
      printHtmlElement(printableRef.current, {
        title: `Employee ID - ${emp.first_name} ${emp.last_name}`,
  ...
  const empIdDisplay = emp.employee_id || emp.id.split('-')[0].toUpperCase()

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-background/95 border-border shadow-2xl">
        <DialogHeader className="no-print">
          ...
          <DialogDescription>
            Ready-to-print dual-side PVC identification badge for Priority Handling Logistics, Inc.
          </DialogDescription>
        </DialogHeader>

        {/* Printable Area containing Front and Back Cards */}
        <div ref={printableRef} className="print-area py-4 flex flex-col sm:flex-row items-center justify-center gap-6 bg-white text-black p-6 rounded-xl">
          {/* FRONT OF BADGE */}
          <div className="w-[2.125in] h-[3.375in] bg-white rounded-xl border border-gray-300 shadow-md overflow-hidden relative flex flex-col shrink-0 print-avoid-break">
            {/* Header Banner */}
            <div className="bg-primary h-14 w-full flex flex-col items-center justify-center pt-1 px-2 text-center text-white">
              <h2 className="font-black text-[9px] tracking-wider uppercase leading-none">PRIORITY HANDLING LOGISTICS</h2>
              <span className="text-[7px] text-white/80 uppercase tracking-widest font-semibold mt-0.5">Corporate ID Pass</span>
            </div>

            {/* Photo Avatar */}
            <div className="absolute top-9 left-1/2 -translate-x-1/2">
              <div className="size-16 bg-gray-100 rounded-full border-2 border-white flex items-center justify-center overflow-hidden shadow-sm">
                {emp.avatar_url ? (
            ...
            {/* Employee Details */}
            <div className="pt-12 px-3 text-center flex-1 flex flex-col">
              <h3 className="font-black text-gray-900 text-xs uppercase tracking-wide leading-tight mt-1">
                {emp.first_name} {emp.last_name}
              </h3>
              <p className="text-[9px] text-primary font-bold mt-0.5 leading-tight">
                {emp.position || 'Logistics Officer'}
              </p>
              <p className="text-[8px] text-gray-600 font-medium uppercase mt-0.5">
                {emp.departments?.name || 'Operations'}
              </p>

              {/* Barcode & ID */}
              <div className="mt-auto pb-3 flex flex-col items-center">
                <div className="w-full flex justify-center gap-0.5 mb-1 opacity-80">
                  {Array.from({ length: 28 }).map((_, i) => (
                    <div key={i} className={`bg-gray-900 h-5 ${i % 3 === 0 ? 'w-1' : i % 2 === 0 ? 'w-0.5' : 'w-px'}`} />
                  ))}
                </div>
                <p className="text-[8px] text-gray-700 font-mono font-bold tracking-widest">
                  ID: {empIdDisplay}
                </p>
              </div>
            </div>
          ...
          {/* BACK OF BADGE */}
          <div className="w-[2.125in] h-[3.375in] bg-white rounded-xl border border-gray-300 shadow-md overflow-hidden relative flex flex-col p-3 text-[7.5px] text-gray-700 leading-tight shrink-0 print-avoid-break">
            <div className="text-center border-b border-gray-300 pb-1.5 mb-1.5">
              <p className="font-bold text-gray-900 uppercase text-[8px]">Terms & Conditions</p>
              <p className="text-gray-500 text-[6.5px]">This card is property of Priority Handling Logistics, Inc.</p>
            </div>

            <div className="space-y-1 text-[7px] text-gray-600">
              <p>• Must be worn within company facilities and logistics terminals at all times.</p>
              <p>• If found, return to HR Dept, Pasay City, Metro Manila.</p>
              <p>• Hotline: <span className="font-bold text-gray-900">+63 (02) 8888-0000</span></p>
            </div>

            <div className="my-auto border-t border-gray-200 pt-1.5 space-y-0.5">
              <p className="font-bold text-gray-900">EMERGENCY CONTACT:</p>
              <p className="text-gray-700 font-medium">HR Support: +63 917 123 4567</p>
              <p className="text-[6.5px] text-gray-500">DOLE Compliant &bull; SSS / PhilHealth / HDMF Registered</p>
            </div>

            <div className="mt-auto border-t border-gray-300 pt-1 text-center">
              <div className="h-3 flex items-center justify-center">
                <span className="font-serif italic text-gray-400 text-[9px]">Authorized Signatory</span>
              </div>
              <p className="font-bold uppercase text-[7px] text-gray-900">Managing Director / HR Head</p>
            </div>
        ...
        <DialogFooter className="no-print">
          ...
          <Button className="gap-2" onClick={handlePrint}>
            <Printer className="size-4" /> Print ID Badges
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function EmployeesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: rpcResult, error: rpcError } = await supabase.rpc('create_employee_account', {
        p_email: form.email,
        p_password: form.password,
        p_first_name: form.first_name,
        p_last_name: form.last_name || '',
        p_phone: form.phone || null,
        p_gender: form.gender || 'unspecified',
        ...
        p_salary_info: salaryInfo,
        p_emergency_contact: emergencyContact,
      })

      if (rpcError) {
        // If RPC function not found or failed, attempt edge function invocation as fallback
        ...
        if (invokeError) {
          throw new Error(rpcError.message || invokeError.message || 'Failed to create employee')
        }
