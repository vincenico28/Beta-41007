...
An enterprise-grade, intelligent, and highly secure human resources and workforce management platform. Engineered specifically for **Priority Handling Logistics, Inc.**, this platform integrates **deep-learning facial biometric verification**, **strict high-precision GPS geofencing & dual-verification enforcement**, **velocity anomaly & impossible-travel detection**, **mandatory multi-stage authentication for leadership**, an **embedded Gemini AI HR Assistant**, **Philippine DOLE / BIR Statutory Compliance Suites**, **Mandatory Schedule Compliance & Late / Early Deduction Engines**, **Multi-Stage Leave Management with Unused Leave Cash Monetization (DOLE Art. 95)**, an **Enterprise HR Performance Appraisal & Review Suite**, an **Automated AI Roster & Shift Scheduler**, **Real-Time Daily Attendance Clock-In Spatial Mapping**, and an **Ultra-Responsive Mobile Web UI Architecture** into a modern, unified web application.

---

## 🌟 The "AI & Intelligence" Advantage

Unlike traditional HR software, this system actively monitors, verifies, and analyzes workforce data to prevent fraud, eliminate buddy-punching, enforce strict GPS precision, and assist management with real-time operations.
...
### 3. 📍 Strict GPS Geofencing, Precision Lock & Dual-Verification Enforcement
- **Hardware-Level Precision Standard ($\le 35\text{m}$):** Queries real-time hardware GPS (`enableHighAccuracy: true`, `timeout: 12000`, `maximumAge: 0`). Readings with an error margin exceeding $35\text{m}$ (e.g. coarse cell-tower triangulation) are strictly rejected as unreliable.
...
- **Mandatory Full Process Repetition Policy:**
  - Biometric face recognition and GPS location verification are atomically linked.
  - If the GPS signal is inaccurate, timed out, or outside the authorized geofence boundary, the entire attempt is **immediately invalidated**.
  - The employee is alerted via a dedicated **GPS Verification Alert Dialog** and **must repeat the entire clock-in process from scratch** (both Face Verification and GPS Location Verification). No partial or unverified records are accepted.
- **Visual GPS Uncertainty Radius:** Live Geofence map displays an interactive blue uncertainty circle corresponding to the exact device GPS accuracy ($\pm X\text{m}$).
...
- **Authentic Verified Locations Only:** Strictly maps authentic, verified device coordinates with zero synthetic fallbacks or fake pins.
- **High-Precision Multi-Format GPS:** Displays exact device coordinates, $\pm$ accuracy radius, timestamps, and direct Google Maps verification links.
...
### ⏱️ 3. Mandatory Schedule Compliance & Deduction Engine
- **DOLE Schedule Adherence:** Enforces scheduled shift start/end times across Attendance, Timesheet, and Payroll records.
- **Late Clock-In (Tardiness) Deduction:**
  - Automatically compares clock-in time with shift start time.
  - Computes exact tardiness minutes and per-minute monetary deduction:
    $$\text{Tardiness Deduction} = \frac{\text{Hourly Rate}}{60} \times \text{Tardiness Minutes}$$
- **Early Clock-Out (Undertime) Deduction:**
  - Flags departures prior to scheduled shift end.
  - Computes exact undertime minutes and per-minute monetary deduction:
    $$\text{Undertime Deduction} = \frac{\text{Hourly Rate}}{60} \times \text{Undertime Minutes}$$
- **Official Civil Service Form 48 (DTR) Integration:**
  - Itemizes daily arrival/departure, regular/overtime hours, late/undertime minutes, and exact penalty deductions on printable DTRs.
- **Payroll Deduction Line Items:**
  - Transparently itemizes Tardiness and Undertime deductions on employee payslips and bank CSV exports.

### 🌴 4. Philippine DOLE Statutory Leaves & Year-End Monetization Suite
...
### 🇵🇭 5. Philippine DOLE & BIR Statutory Payroll System
- **Philippine Statutory Contribution Engine:**
  ...
  - **Schedule Penalties Integration:** Automatic deduction of Tardiness and Undertime against gross pay.
...
### 🎖️ 6. Enterprise HR Performance Appraisal & Review Suite
- **3-Step HR Performance Appraisal Workflow:**
  1. **Supervisor Evaluation:** Evaluates 5 core competencies on a 5.0 scale (Job Knowledge, Quality, Attendance, Teamwork, Initiative) with review cycles (Quarterly, Semi-Annual, Annual, Probationary, Project), strengths, and SMART goals.
  2. **Employee Digital Acknowledgement:** Employees view evaluations in their personal portal, provide self-reflection feedback, and digitally sign & acknowledge.
...
### 📅 7. HR Shift Scheduling & Roster Management Suite
...
### ⏱️ 8. Biometric Timesheet & Overtime Management Suite
...
- **⚡ 1-by-1 & Bulk Overtime Approval:** Fast-track individual overtime requests (`Approve OT (+X.Xh)`) or bulk-approve all unverified entries across the workforce.
...
- **🔍 Multi-Level Department & Status Filters:** Filter by Department, Employee, or Status (*All Logs*, *Pending Approval*, *Approved*, *Pending OT ({count})*).

### 🔔 9. Realtime Notification Broadcaster & Sound Engine
...
### 👥 10. Employee Directory & 201 File Management
...
- **Cascading Foreign Key Integrity:** Clean database relationship enforcement (`ON DELETE CASCADE`) preventing foreign key lockouts and orphaned records during attendance deletions.
